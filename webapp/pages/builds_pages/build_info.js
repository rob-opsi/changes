...
export const SingleBuild = React.createClass({
  ...
  componentDidMount() {
    ...
    var job_ids = this.props.build.jobs.map(j => j.id);
  ...
  render() {
    ...
    var job_phases = {};
    Object.keys(this.state.jobPhases).forEach(k => {
      job_phases[k] = this.state.jobPhases[k].getReturnedData();
    });
  ...
  renderBuildMessages(buildMessages) {
    if (buildMessages.length == 0) {
      return null;
    }
    let messageDisplays = buildMessages.map(message => {
      return (
  ...
  renderCoverage(coverageInfo) {
    let paths = Object.keys(coverageInfo);
    if (paths.length == 0) {
      return null;
    }
  ...
  renderButtons(build) {
    ...
    if (build.tags.indexOf('commit-queue') !== -1) {
      recreateButton = (
  ...
  renderFailedTests(build) {
    if (build.testFailures.total <= 0) {
      return null;
    }

    var rows = [];
    build.testFailures.tests.forEach(test => {
  ...
  renderJobs(build, phases) {
    var markup = build.jobs.map((job, index) => {
  ...
  renderJobTable(job, build, all_phases) {
    var failures = build.failures.filter(f => f.job_id == job.id);
    ...
    var phases_rows = phases.map(phase => {
      ...
      while (remaining) {
        remaining_steps = remaining_steps.filter(step => {
      ...
      for (let index = 0; index < grouped_steps.length; index++) {
        ...
        var jobstep_failures = failures.filter(f => f.step_id == jobstep.id);
        if (jobstep_failures) {
          var innerFailureMarkup = jobstep_failures.map(f => {
            var reason = f.reason;
        ...
        let logSources = _.sortBy(jobstep.logSources, ls => [
          logSourceMaxPriority(ls) || 0,
          ls.name
        ]).reverse();
        var links = [];
