...
import {Error, AjaxError} from 'display/errors';
...
function logging_endpoint(url, offset = null) {
  ...
  return URI(url).setSearch(params).toString();
}
...
var LogPage = React.createClass({
  ...
  render: function() {
    if (!api.allLoaded([this.state.job, this.state.build])) {
      return <APINotLoadedPage calls={[this.state.job, this.state.build]} fixed={true} />;
    }
...
var LogComponent = React.createClass({
  ...
  render: function() {
    ...
    var apiCallsToRender = [initialLog.getReturnedData()].concat(this.state.newLogs);
  ...
  renderDebugContent() {
      ...
      _.each(call.chunks, chunk => {
        var icon =
          chunk.offset === expectedOffset
            ...
            : <SimpleTooltip label="This offset doesn't look right. Did we not render some log lines?">
    ...
    return (
      <div>
        ...
        <Grid colnum={3} data={data} headers={['Offset', 'Chunk', 'Source']} />
        This debugging info may not help you diagnose cases where we prematurely stop
        fetching log lines before the log is complete.
      </div>
    );
  },
  ...
  renderTopRightBox() {
    ...
    return (
      <div className="logReturnLink" style={style}>
        {header}
        <div>
          {this.state.linesRendered} lines &middot;{' '}
          <a href={ChangesLinks.buildHref(build)}>Return to build</a>
        </div>
  ...
  pollForUpdates: function() {
    ...
    api.make_api_ajax_get(
      logging_endpoint(this.props.initialLogURL, latestData.nextOffset),
      null,
      handle_response,
      handle_response
    );
