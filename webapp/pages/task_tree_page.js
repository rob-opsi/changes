...
import {get_runnable_condition, ConditionDot} from 'display/changes/build_conditions';
...
var TaskTreePage = React.createClass({
  ...
  render: function() {
    if (!api.allLoaded([this.state.objectTasks])) {
      return <APINotLoadedPage calls={[this.state.objectTasks]} />;
    }
    ...
    if (data.length == 0) {
      content = (
        <div>
          No tasks found; they may have been cleaned up, or the supplied id may be
          invalid.
        </div>
      );
    }
    return (
      <ChangesPage>
        <SectionHeader>
          Task Tree for {this.props.objectID}
        </SectionHeader>
        {content}
      </ChangesPage>
    );
...
var TaskTree = React.createClass({
  ...
  renderTasks: function(t) {
    ...
    return (
      <span>
        <div>
          <ConditionDot condition={get_runnable_condition(t)} size="smaller" />
          {name}{' '}
          <i style={{color: 'gray'}}>[modified {moment.utc(t.dateModified).fromNow()}]</i>
        </div>
        {kids}
      </span>
    );
  ...
  render: function() {
    if (!api.allLoaded([this.state.taskTree])) {
      return <APINotLoadedPage calls={[this.state.taskTree]} />;
    }
