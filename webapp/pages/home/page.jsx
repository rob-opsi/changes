...
export default React.createClass({
  ...
  render: function() {
    ...
    if (!api.isLoaded(this.state.commits)) {
      ...
      if (
        api.isError(this.state.commits) &&
    ...
    if (this.props.author) {
      ...
      header_markup = (
        <div className="nonFixedClass" style={{paddingBottom: 30, paddingTop: 10}}>
          Diffs and Commits by {utils.email_head(this.props.author)}
          {full_name}
        </div>
      );
    ...
    return (
      <ChangesPage highlight="My Changes" oldUI="/projects">
        ...
        <div className="nonFixedClass">
          ...
          <Commits commits={this.state.commits} author={this.props.author} />
        </div>
        {projects}
        {options}
      </ChangesPage>
  ...
  render: function() {
    ...
    var grid_data = diffs.map(d => {
      ...
      if (d.builds.length > 0) {
        ...
        title = (
          <a className="subtle" href={ChangesLinks.buildsHref(d.builds)}>
            {title}
          </a>
        );
      }
      ...
      return [
        ...
        <a className="external" href={d['uri']} target="_blank">
          {ident}
        </a>,
    ...
    var headers = ['Result', 'Name', 'Status', 'Diff', 'Updated'];
    ...
    return (
      <div className="paddingBottomM">
        ...
        <Grid colnum={5} data={grid_data} cellClasses={cellClasses} headers={headers} />
      </div>
    );
  },
...
var Commits = React.createClass({
  ...
  render: function() {
    ...
    var header_text = !this.props.author ? 'My Commits' : 'Commits';
    ...
    commits.forEach(c => {
      ...
      if (c.builds.length > 0) {
        name = (
          <a className="subtle" href={ChangesLinks.buildsHref(c.builds)}>
            {name}
          </a>
        );
      }
    ...
    var cellClasses = ['buildWidgetCell', 'wide easyClick', 'nowrap', 'nowrap', 'nowrap'];
    var headers = ['Result', 'Name', 'Project(s)', 'Commit', 'Committed'];
    ...
    if (is_it_out_link && !this.props.author) {
      is_it_out_markup = (
        <div className="darkGray marginTopM">
          ...
          <a className="external" href={is_it_out_link} target="_blank">
            is it out?
          </a>{' '}
        </div>
      );
    ...
    var builds_sentence = !this.props.author ? 'See all of your ' : 'See all ';
    ...
    return (
      <div className="marginTopL">
        ...
        <div className="darkGray marginTopM">
          {builds_sentence}
          <a href={buildsHref}>recent builds</a>
          {'.'}
        </div>
        {is_it_out_markup}
      </div>
