...
import {ConditionDot, get_runnable_condition} from 'display/changes/build_conditions';
...
var TestHistoryPage = React.createClass({
  ...
  componentWillMount: function() {
    ...
    var historyEndpoint =
      `/api/0/projects/${projectID}/tests/${testHash}/history/` +
      '?per_page=100&branch=master';

    this.setState({
      history: InteractiveData(this, 'history', historyEndpoint)
    });
  },
  ...
  renderHistory() {
    ...
    var rows = _.map(history, t => {
      if (!t) {
        return [null, null, null, null, null, <i>Not run for this commit</i>, null];
      }
    ...
    let helpful_header = (text, help) =>
      <SimpleTooltip label={help} placement="top">
        <span>
          {text}
        </span>
      </SimpleTooltip>;
    ...
    if (historyInteractive.failedToLoadUpdatedData()) {
      errorMessage = (
        <AjaxError response={historyInteractive.getDataForErrorMessage().response} />
      );
    }
  ...
  render() {
    ...
    return (
      <ChangesPage>
        ...
        <div className="marginTopM">{this.renderHistory()}</div>
      </ChangesPage>
    );
  }
});
