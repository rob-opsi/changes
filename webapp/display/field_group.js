...
export const redirectCallback = function(fieldGroup, redirectUrl) {
...
export const DiffFormMixin = {
  ...
  updateSavedFormState: function() {
    if (this.getFieldNames === undefined) {
      throw Error(
        'getFieldNames() must be defined on the DiffFormMixin ' +
...
export const create = function(
    ...
    let sectionMarkup = _.map(section.fields, field => {
      if (field.type === 'text' || field.type === 'textarea' || field.type === 'select') {
        ...
        if (field.type === 'text') {
          ...
          let options = _.map(field.options, (option, name) =>
            <option value={option}>
              {name}
            </option>
          );
        ...
        return (
          <div className="marginBottomS">
            <label>
              <div>
                <input type="checkbox" checkedLink={checkedLink} /> {field.comment}{' '}
              </div>
            </label>
            <hr />
          </div>
  ...
  let saveButton = (
    <Button onClick={onSaveClicked} className={saveDisabled}>
      {saveButtonText}
    </Button>
  );
  if (!hasChanges) {
    saveButton = (
      <SimpleTooltip label="There are no pending changes to save.">
        {saveButton}
      </SimpleTooltip>
    );
  }
