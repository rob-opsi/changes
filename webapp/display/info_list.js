...
import {Error, ProgrammingError} from 'display/errors';
...
export const InfoList = React.createClass({
  ...
  render: function() {
    ...
    React.Children.forEach(this.props.children, c => {
      ...
      if (c.props.tooltip) {
        label = (
          <SimpleTooltip label={c.props.tooltip} placement="right">
            <div>
              {c.props.label}
            </div>
          </SimpleTooltip>
      ...
      rows.push(
        <tr>
          ...
          <td className={c.props.valueClassName}>
            {c.props.children}
          </td>
        </tr>
      );
...
export const InfoItem = React.createClass({
  propTypes: {
